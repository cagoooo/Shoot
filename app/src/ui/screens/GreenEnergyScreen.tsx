...
export function GreenEnergyScreen({ learningMode, comfortSettings, onComfortSettingsChange, onBack, onMissionComplete, onAudioSceneChange, mapSlot }: GreenEnergyScreenProps) {
  ...
  const sceneFactory = useCallback<SceneFactory>((engine, runtimeInput, runtimeComfort) => buildGreenEnergyScene(engine as AbstractEngine, runtimeInput ?? inputManager, runtimeComfort), [inputManager])
  ...
  return <main className={`mission-screen${comfortSettings.largeText ? ' large-text' : ''}`}>
    ...
    <div className="mission-layout">
      ...
      <section className="mission-task-card" aria-live="polite">
        <aside className="mission-guide" aria-label="綠能任務圖卡引導"><span className="mission-guide-icon" aria-hidden="true">{guide[phase].icon}</span><div><strong>現在要做什麼？</strong><p>{guide[phase].now}</p></div>{learningMode === 'middle-assist' && <div className="mission-guide-next"><strong>下一步</strong><p>先看資料，再選一個最合適的方法。</p></div>}<div className="mission-guide-learn"><strong>小小科學發現</strong><p>{guide[phase].learn}</p></div></aside>
        ...
        {phase === 'weather' && <><p className="eyebrow">任務 2／4</p><h2>閱讀天氣資料</h2><p>現在是晴朗、微風的下午：陽光充足，風力較弱。</p><div className="route-options water-options"><button type="button" aria-pressed={source === 'solar'} onClick={() => setSource('solar')}><strong>太陽能板</strong><span>晴天時能收集較多能量</span></button><button type="button" aria-pressed={source === 'wind'} onClick={() => setSource('wind')}><strong>風力塔</strong><span>需要較強的風才會轉得快</span></button></div>{source && <button className="primary-button" type="button" onClick={() => setPhase('storage')}>確認發電方式</button>}</>}
        {phase === 'storage' && <><p className="eyebrow">任務 3／4</p><h2>啟動社區電池</h2><p>{source === 'solar' ? '太陽能板收集到許多能量，快把多出的部分存進電池。' : '風力不強，收集到的能量不多；先把它安全存起來。'}</p><button className="primary-button" type="button" onClick={() => setStored(true)} disabled={stored}>儲存綠色能量</button>{stored && <button className="primary-button" type="button" onClick={() => setPhase('balance')}>安排晚上的用電</button>}</>}
        {phase === 'balance' && <><p className="eyebrow">任務 4／4</p><h2>公平安排用電</h2><p>選兩個最需要先有電的地方：醫療站、交通號誌和社區廣告牆。</p><div className="route-options water-options">{['醫療站', '交通號誌', '社區廣告牆'].map((name) => <button key={name} type="button" aria-pressed={priorities.includes(name)} onClick={() => togglePriority(name)}><strong>{name}</strong><span>安排一份儲存的能量</span></button>)}</div>{priorities.length >= 2 && <button className="primary-button" type="button" onClick={() => setPhase('report')}>完成綠能社區行動</button>}</>}
